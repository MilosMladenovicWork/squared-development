...
describe('WavyBottomBackground', () => {
  const WavyBottomBackgroundWithTheme = ({...props}) => <ThemeProvider theme={theme}>
  ...
  describe('IllustrationBody', () => {
    it('accepts fillColor prop - valid', () => {
      const {getByTestId} = render(<WavyBottomBackgroundWithTheme fillColor="accent"/>)
      expect(getByTestId('illustration-body')).toHaveStyle(`
        background:${theme.colors["accent"]};
      `)
    })
    it('accepts fillColor prop - invalid', () => {
      const {getByTestId} = render(<WavyBottomBackgroundWithTheme fillColor="something"/>)
      expect(getByTestId('illustration-body')).toHaveStyle(`
